...
/// Convert seconds since UNIX epoch to (Y,M,D,H,M,S)
fn timestamp_to_ymdhms(mut secs: usize) -> (usize, usize, usize, usize, usize, usize) {
    ...
    loop {
        ...
        if secs >= days_in_year * 24 * 60 * 60 {
            secs -= days_in_year * 24 * 60 * 60;
            year += 1;
        } else {
            break;
        }
...
pub fn date(args: &[String]) -> i32 {
    ...
    let _ = sys_gettimeofday(&mut tv, &mut tz);
    let (y, m, d, h, min, s) = timestamp_to_ymdhms(tv.tv_sec as usize);
    println!("{:04}-{:02}-{:02} {:02}:{:02}:{:02}", y, m, d, h, min, s);
